#!/bin/bash
#
#  THIS SCRIPT IS NOT MINE ( CADEN WESLEY ROBERTS ) IT WAS USED FROM: https://github.com/NrgXnat/oasis-scripts/blob/master/download_scans/download_oasis_scans.sh
#================================================================
...
#
#
unset module# Authenticates credentials against NITRC and returns the cookie jar file name. USERNAME and
...
escape_chars_for_URL() {
    ...
    # escape characters
    output=`echo "${input}" | sed -e 's/%/%25/g;' | sed -e 's/ /%20/g; s/</%3C/g; s/>/%3E/g; s/#/%23/g; s/+/%2B/g; s/{/%7B/g; s/}/%7D/g; s/|/%7C/g; s/\\\/%5C/g; s/\^/%5E/g; s/~/%7E/g; s/\[/%5B/g; s/\]/%5D/g; s/\`/%60/g; s/;/%3B/g; s/\//%2F/g; s/?/%3F/g; s/:/%3A/g; s/@/%40/g; s/=/%3D/g; s/&/%26/g; s/\\$/%24/g'`    echo ${output}
}
